...
import numpy as np
from models.nn.kuma_gate import KumaGate
from models.common.util import get_encoder
from models.nn.rcnn import RCNNCell
EPS = 1e-6
        ...
        for h_t, t in zip(h, range(time)):
            ...
            if self.training:
                ...
                if self.strategy == 1:
                    ...
                    # deterministic strategy 2
                    # ! Yu's strategy
                    P1_2 = z_t_dist.cdf(torch.tensor(0.5).cuda())
                    z_t = (P1_2 < 0.5).to(int)

                elif self.strategy == 3:
                    # ! Zexue: use cdf as the p0 and p1
                    # It's actually similar to strategy 1
                    cdf0 = z_t_dist.pdf(h.new_zeros(()))
                    cdf1 = z_t_dist.pdf(h.new_ones(())-EPS)
                    p0=cdf0
                    p1=1-cdf1
                    pc = cdf1-cdf0  # prob. of sampling a continuous value [B, M]
                    zero_one = torch.where(
                        p0 > p1, h.new_zeros([1]),  h.new_ones([1]))
                    z_t = torch.where((pc > p0) & (pc > p1),
                                      z_t_dist.mean(), zero_one)  # [B, M]
